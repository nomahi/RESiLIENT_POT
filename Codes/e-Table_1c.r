...
for(r in 1:K){
 ...
 for(k in 1:Lg){
	...
	gm01b <- cv.glmnet(x=Xb, y=yb, family="gaussian", alpha=ag[k], nfolds = 10)
	mg[k] <- min(gm01b$cvm)
 ...
 best.a <- ag[which.min(mg)]
 
 lasso01b <- cv.glmnet(x=Xb, y=yb, family="gaussian", alpha=best.a, nfolds = 10)

 lambda01b <- lasso01b$lambda.min

 lasso02b <- glmnet(x=Xb, y=yb, family="gaussian", lambda=lambda01b, alpha=best.a)
